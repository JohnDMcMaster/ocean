    ...
    switch (fmt) {
		...
		case GA_LABEL: 
		    *ganno.o.Label.Attributes = '\0';
		    k = dmScanf (dmfp, "%s %s %d %lg %lg %lg %lg %s\n",
			ganno.o.Label.name,
			ganno.o.Label.Class,
...
int     fmt;
{
	...
	case GEO_ANNOTATE: 
		...
		case GA_LINE: 
		    k = dmPrintf (dmfp, "%.8lg %.8lg %.8lg %.8lg %d\n",
			ganno.o.line.x1, ganno.o.line.y1,
			ganno.o.line.x2, ganno.o.line.y2,
			ganno.o.line.mode);
		    if (k < 0) goto writeError;
		...
		case GA_TEXT: 
		/* Need ~-quoted strings, because they can contain spaces. */
		    k = dmPrintf (dmfp, "%.8lg %.8lg %.8lg %.8lg ~%s~\n",
			ganno.o.text.x, ganno.o.text.y,
			ganno.o.text.ax, ganno.o.text.ay,
			ganno.o.text.text);
		    if (k < 0) goto writeError;
		...
		case GA_LABEL: 
		    /* handle empty-string special cases
		     */
		    k = dmPrintf (dmfp, "%s %s %d %.8lg %.8lg %.8lg %.8lg %s\n",
