/* static char *SccsId = "@(#)write_layout.c 3.51 (TU-Delft) 09/09/98"; */
/**********************************************************
...
if (map->layoutstruct && map->layoutstruct->laylabel) {
   if((fp = dmOpenStream (key, "annotations", "w")) == NULL)
      error(FATAL_ERROR, "write_to_nelsis");
   write_labels(map, fp);
   dmCloseStream(fp, COMPLETE);
}
...
for(box = lay->wire; box != NULL; box = box->next)
   ...
   if(box->layer >= 200)
      ...
      if(box->layer == 200)
         ...
         lwidth = MAX((ViaCellBbx[R][0]  - ViaCellBbx[L][0]), lwidth);
         }
      }
   else
      { /* metal mask */
 ...
 */
static void write_labels(MAPTABLEPTR  map ,
			 DM_STREAM  *fp    /* layout */ 
)
{
register LAYLABELPTR
   hterm;
LAYOUTPTR
   lay;
...
for(hterm = lay->laylabel; hterm != NULL; hterm = hterm->next)
   { 
   if(hterm->layer < 0 || hterm->layer >= Chip_num_layer)
      {
